...
#这是一个句柄
def file_handler(backend_data,res=None,type='fetch'):#默认type设置为fetch
    if type == 'fetch':
        with open('haproxy.conf','r') as read_f:#使用with as方法打开文件，是常用方法
            tag=False
            ret=[]
            ...
            for read_line in read_f:  # server
                ...
                if not tag:
                    ...
                    if not has_write:
                        ...
                        has_write = True
        os.rename('haproxy.conf', 'haproxy.conf.bak')#os模块的文件重命名功能
...
if __name__ == '__main__':
    ...
    while True:
        ...
        res=msg_dic[choice](data)#请牢记这种函数使用方法，在多个函数中通过判断来提取函数名，然后加上括号就可以。
        print('最终结果是--》',res)
